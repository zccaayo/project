...
from project.fireworks import powerups
from project.fireworks.core import MDFW
from project.util import recursive_update
...
def get_converge_wf(
    ...
    **kwargs,
    ...
    wf_name = kwargs.get(
        "wf_name", f"{structure.composition.reduced_formula}_{temperature}_diffusion"
    )
    ...
    _spawner_args = {
        "converge_params": {
            ...
            "converge_type": kwargs.get(
                "convergence_criteria", [("density", 5), ("ionic", 0.001)]
            ),
        },
    ...
    if not preconverged:
        ...
        if converge_scheme == "EOS":
            ...
            EOS_run_args = recursive_update(
                EOS_run_args, kwargs.get("converge_args", {})
            )
            volume_fws = []
    ...
    insert_prod_data = (
        True if save_data == "all" or save_data == "production" else False
    )
    prod_steps = 0
    while prod_steps <= target_steps - max_steps:
        ...
        fw = MDFW(
            structure=structure,
            name=f"{temperature}_prod_run_{prod_count}-{tag_id}",
